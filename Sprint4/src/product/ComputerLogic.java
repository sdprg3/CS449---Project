...
public class ComputerLogic {
    ...
    public void processComputerMove() {
        if (!gameController.getGameProgress()) return; // return if game is not in progress
        ...
        if (gameController.getCurrentPlayer().getPlayerType() == PlayerType.COMPUTER) {
            ...
            if (isValidPosition(position = findOpenHorizontalSOSPattern()) ||
                ...
                pause.setOnFinished(event -> {
                    // Handle the button click after the delay
                    game.getGameLogic().handleButtonClick(row, col, board.getButtonAt(row, col));
                    processComputerMove(); // Recursive call to check for additional moves
                });
    ...
    public int[] findRandomAvailableSpot() {
        ...
        for (int row = 0; row < boardSize; row++) {
            for (int col = 0; col < boardSize; col++) {
                if (boardState[row][col].isEmpty()) {
                    availableSpots.add(new int[]{row, col});
                }
            }
        }
        ...
        int buttonChoice = random.nextInt(2);
        if (buttonChoice == 0) {
            selectSButton();
        } else {
            selectOButton();
